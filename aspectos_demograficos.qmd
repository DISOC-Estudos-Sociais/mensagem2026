...
pop_estados |> 
  mutate(destaque = ifelse(UF == "Ceará", "Ceará", "Outros estados"),
         UF = fct_rev(fct_reorder(UF, total_pop))) |> 
  ggplot(aes(x = UF, y = total_pop, fill = destaque)) +
  geom_col() +
  geom_text(
  ...
  scale_fill_manual(values = c("Ceará" = "#FF6B35", "Outros estados" = "#4A90E2")) +
  scale_y_continuous(
...
piramide_etaria |> 
  ...
  scale_fill_manual(
    values = c("Homem" = "#4A90E2", "Mulher" = "#FF6B35"),
...
sexo_responsavel |> 
  ...
  scale_fill_manual(values = c("Homem" = "#4A90E2", "Mulher" = "#FF6B35")) +
  coord_polar(theta = "y") +
  xlim(c(2, 4)) +
  labs(
